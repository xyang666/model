    ...
    const Eigen::MatrixXd& predictedCov) const
{
    ...
    auto* m = static_cast<const TM_IRMeasurement*>(measurement);
    const auto& input = m->input;

    // ---- 像素 → 本体系 bearing 角 ----
    double az = std::atan2(m->pixelU - cx_, fx_);
    double el = std::atan2(m->pixelV - cy_, fy_);
    double azErr = (m->pixelErrorU > 0) ? m->pixelErrorU : 2.0;
    double elErr = (m->pixelErrorV > 0) ? m->pixelErrorV : 2.0;
    ...
    double rangeErr = range * 0.5;  // IR 距离不确定性更大
