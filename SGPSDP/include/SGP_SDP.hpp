...
namespace SGPSDP
{
    ...
    class SGPSDP_OrbitalPropagator
    {
        SGPSDP_OrbitalPropagator() = default;
        void Initialize();
        ...
        bool SetInitialOrbitalState(const OrbitalState &aState);

    protected:
        void Propagate(const Calendar &aTime);

        void UpdateOrbitalState();

        bool HyperbolicPropagationAllowed() { return false; }
    ...
    private:
        ...
        // data elements
        tle_t *tle{nullptr}; // Kept the original implementation's name (Project Pluto).
    ...
    {
    public:
        // explicit tle_t(const TwoLineElement &aTLE)
        //     : epoch(aTLE.GetEpochDate()), xndt2o(aTLE.GetMeanMotionFirstDerivative()), xndd6o(aTLE.GetMeanMotionSecondDerivative()), bstar(aTLE.GetBstarDrag()), xincl(aTLE.GetInclination()), xnodeo(aTLE.GetRAAN()), eo(aTLE.GetEccentricity()), omegao(aTLE.GetArgumentOfPeriapsis()), xmo(aTLE.GetMeanAnomaly()), xno(aTLE.GetMeanMotion())
        // {
        //     xno *= 60.0; // rad / min
        // }
